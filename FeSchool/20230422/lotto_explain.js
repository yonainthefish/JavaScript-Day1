//balls라는 변수를 만든다,.
//document(문서)에서 "ball"이라는 클래스를 검색하는 명령어를 통해(".ball")
//document.querySelectorAll을 사용하여 이것의 뜻은 문서에서 명령에 맞게 모든 걸 가져와랔
...
for (let i = 0; i < balls.length; i++) {
    ...
    //balls 0번째 뭐가있음?
    //우리가 쿼리셀렉터all해서 가져온 것 중 0번째인것 이걸 balls[0]이라고 부름
    //balls[0]에 onclick을 설정한다. 
    //onclick이라는 행동이 브라우저에서 일어나면 설정한 함수가 실행됨
    balls[i].onclick = function () {


        //설정할 함수내용들 (실행할 코드블럭)
        //prompt라는 함수를 실행함. 
        //promptm는 입력창이 있는 프롬프트를 보여주고
        ///입력하고 확인을 누르면 그 입력한 값을 돌려준다. (return해준다)
        // 이 프롬프트를 총해 받은값을 num에 넣게 된다. 
    ...
    // 그 저장된 값을 가져와서 알려줌.
      ...
      //balls[i]번째 -> (지금은 0 번째) 값을 보여주고, 현재 몇번째인지 보여준다. 
      console.log(balls[i],i)
      
      //balls 0번째 있는 요소 안에 html을 바꿔준다. 
